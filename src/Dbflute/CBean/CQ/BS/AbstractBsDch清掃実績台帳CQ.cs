...
namespace Dbflute.CBean.CQ.BS {
    ...
    [System.Serializable]
    public abstract class AbstractBsDch清掃実績台帳CQ : AbstractConditionQuery {

        public AbstractBsDch清掃実績台帳CQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel)
            : base(childQuery, sqlClause, aliasName, nestLevel) {}

        public override String getTableDbName() { return "dch清掃実績台帳"; }
        public override String getTableSqlName() { return "dch清掃実績台帳"; }
        ...
        //                                                                    Scalar Condition
        //                                                                    ================
        public SSQFunction<Dch清掃実績台帳CB> Scalar_Equal() {
            return xcreateSSQFunction("=");
        }

        public SSQFunction<Dch清掃実績台帳CB> Scalar_NotEqual() {
            return xcreateSSQFunction("<>");
        }

        public SSQFunction<Dch清掃実績台帳CB> Scalar_GreaterEqual() {
            return xcreateSSQFunction(">=");
        }

        public SSQFunction<Dch清掃実績台帳CB> Scalar_GreaterThan() {
            return xcreateSSQFunction(">");
        }

        public SSQFunction<Dch清掃実績台帳CB> Scalar_LessEqual() {
            return xcreateSSQFunction("<=");
        }

        public SSQFunction<Dch清掃実績台帳CB> Scalar_LessThan() {
            return xcreateSSQFunction("<");
        }

        protected SSQFunction<Dch清掃実績台帳CB> xcreateSSQFunction(String operand) {
            return new SSQFunction<Dch清掃実績台帳CB>(delegate(String function, SubQuery<Dch清掃実績台帳CB> subQuery) {
                xscalarSubQuery(function, subQuery, operand);
            });
        }

        protected void xscalarSubQuery(String function, SubQuery<Dch清掃実績台帳CB> subQuery, String operand) {
            assertObjectNotNull("subQuery<Dch清掃実績台帳CB>", subQuery);
            Dch清掃実績台帳CB cb = new Dch清掃実績台帳CB(); cb.xsetupForScalarCondition(this); subQuery.Invoke(cb);
            String subQueryPropertyName = keepScalarSubQuery(cb.Query()); // for saving query-value.
            registerScalarSubQuery(function, cb.Query(), subQueryPropertyName, operand);
        }
        public abstract String keepScalarSubQuery(Dch清掃実績台帳CQ subQuery);

        // ===============================================================================
        //                                                                  MySelf InScope
        //                                                                  ==============
        public void MyselfInScope(SubQuery<Dch清掃実績台帳CB> subQuery) {
            assertObjectNotNull("subQuery<Dch清掃実績台帳CB>", subQuery);
        ...
        public abstract String keepMyselfInScopeSubQuery(Dch清掃実績台帳CQ subQuery);
